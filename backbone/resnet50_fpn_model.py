...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        for _ in range(1, block_num):
            layers.append(block(
                self.in_channel, channel, norm_layer=norm_layer
            ))
...
def resnet50_fpn_backbone(pretrain_path="",
                          ...
                          extra_blocks=None):
    """
    搭建 resnet50_fpn backbone
    :param pretrain_path: resnet50的预训练权重，如果不使用就默认为空
    :param norm_layer: 默认的是FrozenBatchNorm2d，即不会更新参数的bn层
                       (因为如果batch_size设置的很小会导致效果更差，还不如不用bn层)
    ...
    # in_channel_stage2 为layer2的输出特征矩阵channel = 246
    in_channels_stage2 = resnet_backbone.in_channel // 8   # 256
    # 记录resnet50提供给fpn的每个特征层channel
    in_channels_list = [in_channels_stage2 * 2 ** (i - 1) for i in returned_layers]
    # 通过fpn后得到的每个特征层的channel
    out_channels = 256
    return BackboneWithFPN(resnet_backbone, return_layers, in_channels_list,
                           out_channels, extra_blocks=extra_blocks)


if __name__ == '__main__':
    resnet_backbone = ResNet(Bottleneck, [3, 4, 6, 3], include_top=False, norm_layer=FrozenBatchNorm2d)
    resnet_backbone.load_state_dict(torch.load("../weights/resnet50.pth"), strict=False)
